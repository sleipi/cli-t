...
func Entry(entry types.Entry, captures map[string]string) Result {
	...
	result := runner.RunWithEnv(cmd, entry.Directives.Env, entry.Directives.Workdir)
...
func promptEntry(entry types.Entry, cmd string, captures map[string]string) Result {
	...
	pr := runner.RunWithPrompts(cmd, prompts, timeout, entry.Directives.Env, entry.Directives.Workdir)
