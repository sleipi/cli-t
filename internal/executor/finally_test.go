...
func TestExecuteFinally_SignalAndExit(t *testing.T) {
	...
	bp, err := runner.RunBackground(`sh -c 'trap "exit 0" TERM; echo ready; sleep 10'`)
	if err != nil {
...
func TestExecuteFinally_ExitCodeMismatch(t *testing.T) {
	...
	bp, err := runner.RunBackground(`sh -c 'trap "exit 1" TERM; echo ready; sleep 10'`)
	if err != nil {
		t.Fatalf("failed to start: %v", err)
	}
	deadline := time.Now().Add(3 * time.Second)
...
func TestExecuteFinally_Timeout(t *testing.T) {
	...
	bp, err := runner.RunBackground(`sh -c 'trap "" TERM; echo ready; sleep 10'`)
	if err != nil {
		t.Fatalf("failed to start: %v", err)
	}
	deadline := time.Now().Add(3 * time.Second)
...
func TestExecuteFinally_PostSignalAsserts(t *testing.T) {
	...
	bp, err := runner.RunBackground(`sh -c 'trap "echo shutdown >&2; exit 0" TERM; echo ready; sleep 10'`)
	if err != nil {
		t.Fatalf("failed to start: %v", err)
	}
	deadline := time.Now().Add(3 * time.Second)
...
func TestExecuteFinally_LIFO(t *testing.T) {
	bp1, err := runner.RunBackground(`sh -c 'trap "exit 0" TERM; echo r1; sleep 10'`)
	if err != nil {
		t.Fatalf("failed to start bp1: %v", err)
	}
	bp2, err := runner.RunBackground(`sh -c 'trap "exit 0" TERM; echo r2; sleep 10'`)
	if err != nil {
